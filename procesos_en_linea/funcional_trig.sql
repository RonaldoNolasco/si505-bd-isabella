	...
	IF NEW.id_tipo_estado_pedido = 2 THEN
			...
			WHILE i < n DO
				...
				WHILE j < m DO
					...
					ELSEIF idCategoriaProducto = 6 THEN
						INSERT INTO contenedor (id_tipo_contenedor, id_almacen) VALUES (idProducto-9, 1);
					END IF;
					SET j = j + 1;
				END WHILE;
				SET i = i + 1;
		...
		IF tipoPedido = "Venta" THEN 
			WHILE i < n DO
				...
				AND id_tipo_estado_item = 1 AND fecha_fin IS NULL) < m THEN -- Falta hacer inner join con estado item
					SET valid = FALSE;
				END IF;
				SET i = i + 1;
			END WHILE;
			...
			IF valid = TRUE THEN 
				WHILE i < n DO
					...
					WHILE j < m DO
						...
						AND id_tipo_estado_item = 1 and fecha_fin is null ORDER BY fecha_inicio ASC LIMIT 1);
						INSERT INTO estado_item (id_item, id_tipo_estado_item, fecha_inicio, fecha_fin) VALUES (idItem, 2, CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP());-- Insertar el estado de vendido
						UPDATE estado_item SET fecha_fin = CURRENT_TIMESTAMP() WHERE id_item = idItem AND id_tipo_estado_item <> 2;-- Terminar los otros estados del item
						SET j = j + 1;
					END WHILE;
					SET i = i + 1;
					SET j = 0;
